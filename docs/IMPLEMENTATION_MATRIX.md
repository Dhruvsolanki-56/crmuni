...
| Requirement                              | Status  | Evidence / gap                                                                                                                                                                    |
| ---------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Manual/card/badge/QR/audio capture       | PARTIAL | Card, badge and QR images are read before Save with bundled on-device English Tesseract OCR plus QR/vCard decoding, with no OCR API key. The deterministic sample uses the same path. Audio capture exists, but transcription and proprietary badge compatibility still require separate validation. |
...
| OCR confidence and correction            | COMPLETE | Local OCR/QR suggestions prefill the editable capture form before Save; reviewed fields and reviewer attribution are stored with the original image. Server extraction suggestions remain separately reviewable. Human review is mandatory because local OCR accuracy varies with image quality and layout. |
