...
export async function POST(request: Request) {
  ...
  statements.push(database().prepare(`INSERT INTO lead_assignment_history (id,workspace_id,lead_id,previous_owner_id,owner_id,reason,changed_by,created_at) VALUES (?,?,?,NULL,?,'captured_by',?,?)`).bind(crypto.randomUUID(),context.workspace.id,leadId,context.user.id,context.user.id,now));
