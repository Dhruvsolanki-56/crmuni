...
import { database, DEFAULT_WORKSPACE, requestUser, revenueEnv } from '@/lib/db';
...
export async function POST(request: Request) {
  ...
  const source = await database().prepare(`
    ...
    WHERE l.id = ? AND l.workspace_id = ? AND l.owner_id = ?
    ORDER BY i.created_at DESC LIMIT 1
  `).bind(leadId, DEFAULT_WORKSPACE, user.id).first<{ id: string; fullName: string; company: string; role: string | null; interactionId: string; note: string }>();
  ...
  `).bind(extractionId, DEFAULT_WORKSPACE, leadId, source.interactionId, model, now).run();
  ...
  try {
    ...
    await database().prepare(`UPDATE ai_extractions SET status = 'completed', result_json = ?, completed_at = ? WHERE id = ? AND workspace_id = ?`).bind(JSON.stringify(analysis), Date.now(), extractionId, DEFAULT_WORKSPACE).run();
    ...
    await database().prepare(`UPDATE ai_extractions SET status = 'failed', error_code = ?, completed_at = ? WHERE id = ? AND workspace_id = ?`).bind(code, Date.now(), extractionId, DEFAULT_WORKSPACE).run();
