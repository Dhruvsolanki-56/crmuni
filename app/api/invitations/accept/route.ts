...
export async function POST(request: Request) {
  ...
  if (workspace.plan === 'trial' && invitation.status === 'pending') {
  ...
  const now = Date.now();
  const accepted = invitation.status === 'pending' ? await db.prepare(`UPDATE invitations SET status='accepted' WHERE id=? AND status='pending'`).bind(invitationId).run() : null;
  await db.prepare(`INSERT INTO memberships (id,workspace_id,user_id,email,display_name,role,status,created_at,updated_at) VALUES (?,?,?,?,?,?,'active',?,?) ON CONFLICT(workspace_id,user_id) DO UPDATE SET email=excluded.email,role=excluded.role,status='active',updated_at=excluded.updated_at`).bind(crypto.randomUUID(),invitation.workspaceId,user.id,user.email,user.email.split('@')[0],invitation.role,now,now).run();
  if (accepted?.meta.changes) await db.prepare(`INSERT INTO audit_events (id,workspace_id,actor_id,action,entity_type,entity_id,created_at) VALUES (?,?,?,'invitation.accepted','invitation',?,?)`).bind(crypto.randomUUID(),invitation.workspaceId,user.id,invitationId,now).run();
  return Response.json({ ok: true, duplicate: !accepted?.meta.changes, workspaceId: invitation.workspaceId });
}
