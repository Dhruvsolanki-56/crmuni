...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get('export') === '1') {
    ...
    const tables = ['memberships','invitations','accounts','leads','lead_consents','suppression_entries','lead_duplicate_suggestions','lead_merge_events','lead_qualification_history','lead_assignment_history','account_stakeholders','lead_capture_assets','interactions','tasks','task_history','meetings','meeting_participants','meeting_history','ai_extractions','lead_facts','qualification_scores','communication_drafts','rfqs','rfq_items','rfq_documents','rfq_ai_extractions','rfq_history','rfq_submissions','opportunities','opportunity_contacts','opportunity_history','quotations','quotation_revisions','quotation_history','company_profiles','products','ideal_customer_profiles','qualification_rules','knowledge_sources','events','event_memberships'] as const;
...
export async function POST(request: Request) {
  ...
  if (action === 'execute_deletion') {
    ...
    const deleteOrder=['lead_facts','qualification_scores','communication_drafts','lead_consents','suppression_entries','lead_duplicate_suggestions','lead_merge_events','lead_qualification_history','lead_assignment_history','meeting_participants','meeting_history','meetings','task_history','ai_extractions','tasks','interactions','account_stakeholders','lead_capture_assets','quotation_history','quotation_revisions','quotations','rfq_ai_extractions','rfq_items','rfq_documents','rfq_submissions','rfq_history','rfqs','opportunity_contacts','opportunity_history','opportunities','leads','accounts','event_memberships','events','company_documents','knowledge_sources','qualification_rules','ideal_customer_profiles','products','company_profiles','request_rate_limits','invitations','audit_events','workspace_deletion_requests','memberships'] as const;
