import { auditStatement, database, eventAccessClause, requireEventAccess, requireOpportunityAccess, requireQuotationAccess, requireRfqAccess, requireRole, requireWorkspace, revenueEnv } from '@/lib/db';
...
export async function GET(request: Request) {
  ...
  const access=eventAccessClause(context,'q.event_id'); const rows = await db.prepare(`SELECT q.id,q.event_id AS eventId,q.rfq_id AS rfqId,q.opportunity_id AS opportunityId,q.quote_number AS quoteNumber,q.customer,q.amount,q.currency,q.valid_until AS validUntil,q.status,CASE WHEN q.storage_key IS NULL THEN 0 ELSE 1 END AS hasDocument,q.original_name AS originalName,q.created_at AS createdAt FROM quotations q WHERE q.workspace_id=?${access.sql} ORDER BY q.created_at DESC LIMIT 100`).bind(context.workspace.id,...access.bindings).all(); return Response.json({ quotations: rows.results });
}
...
export async function POST(request: Request) {
  ...
  let eventId=clean(request.headers.get('x-revenue-event-id'),80); if(rfqId){const rfq=await requireRfqAccess(context,rfqId);if(eventId&&eventId!==rfq.eventId)return Response.json({error:'The RFQ belongs to a different event.'},{status:409});eventId=rfq.eventId||'';} if(opportunityId){const opportunity=await requireOpportunityAccess(context,opportunityId);if(eventId&&eventId!==opportunity.eventId)return Response.json({error:'The opportunity belongs to a different event.'},{status:409});eventId=opportunity.eventId||'';} await requireEventAccess(context,eventId);
  ...
  try {
    await db.batch([db.prepare(`INSERT INTO quotations (id,workspace_id,event_id,rfq_id,opportunity_id,quote_number,customer,amount,currency,valid_until,status,original_name,storage_key,content_type,size_bytes,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,'draft',?,?,?,?,?,?,?)`).bind(id, context.workspace.id, eventId, rfqId || null, opportunityId || null, quoteNumber, customer, amount, context.workspace.currency, validUntil || null, file?.name.slice(0, 180) || null, storageKey, file?.type || null, file?.size || null, context.user.id, now, now), auditStatement(context, 'quotation.created', 'quotation', id, { quoteNumber, amount, rfqId: rfqId || null, eventId })]);
  } catch (error) { if (storageKey) await revenueEnv().FILES.delete(storageKey); throw error; }
  return Response.json({ quotation: { id, eventId, rfqId: rfqId || null, opportunityId: opportunityId || null, quoteNumber, customer, amount, currency: context.workspace.currency, validUntil: validUntil || null, status: 'draft', hasDocument: Boolean(file), originalName: file?.name || null, createdAt: now } }, { status: 201 });
}
