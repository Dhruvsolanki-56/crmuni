...
export async function GET(request: Request) {
  ...
  const [leadRows, taskRows, opportunityRows, accountRows] = await Promise.all([
    db.prepare(`SELECT l.id, l.event_id AS eventId, l.account_id AS accountId, l.full_name AS fullName, l.company, l.role, l.email, l.phone, s.buying_role AS buyingRole, l.review_status AS reviewStatus,
      ...
      WHERE l.workspace_id = ?${leadAccess.sql} ORDER BY l.created_at DESC LIMIT 100`).bind(context.workspace.id, ...leadAccess.bindings).all(),
    ...
    db.prepare(`SELECT a.id, a.name AS company, a.normalized_name AS normalizedName, COUNT(l.id) AS contacts, MAX(l.created_at) AS latestAt, COUNT(s.id) AS stakeholders FROM accounts a JOIN leads l ON l.account_id=a.id LEFT JOIN account_stakeholders s ON s.lead_id=l.id WHERE a.workspace_id=?${leadAccess.sql} GROUP BY a.id, a.name, a.normalized_name ORDER BY latestAt DESC`).bind(context.workspace.id, ...leadAccess.bindings).all(),
  ]);
