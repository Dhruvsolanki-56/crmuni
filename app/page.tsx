...
type RfqItem = { id: string; title: string; reference?: string; requesterCompany: string; contactName?: string; deliveryLocation?: string; submissionDeadline?: string; status: string; processingStatus: string; ownerId: string; itemCount: number; documentCount: number; createdAt: number };
...
export default function Home() {
  ...
  function go(view: View) { setActiveView(view); setMobileNav(false); if (view === 'settings') void loadSettings(); if (view === 'knowledge') void loadKnowledge(); if (view === 'rfqs') void loadRfqs(); if (view === 'roi') void loadEvents(); if (view === 'events') { void loadEvents(); void loadSettings(); } }
      ...
      <section className="workspace">
        ...
        <div className="content">
          ...
          <section className="panel leads-panel">
            ...
            {activeView === 'opportunities' ? <article className="panel data-panel">{opportunities.length ? <><div className="data-header"><span>Opportunity</span><span>Stage</span><span>Value</span><span>Probability</span></div>{opportunities.map((item) => <div className="data-row" key={item.id}><span><strong>{item.title}</strong><small>{item.company}</small></span><span className="stage-chip">{item.stage}</span><span>{money(item.value, item.currency)}</span><span>{item.probability}%</span></div>)}</> : <div className="empty-state large"><Target /><h2>No opportunities yet</h2><p>Convert a qualified conversation into your first pipeline record.</p><Button onClick={() => setOpportunityOpen(true)}>Create opportunity</Button></div>}</article> : null}
