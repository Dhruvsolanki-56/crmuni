...
type Opportunity = { id: string; eventId?: string; leadId?: string; company: string; title: string; stage: string; value: number; currency: string; probability: number; expectedCloseDate?: string };
...
export default function Home() {
  ...
  async function loadSettings() { const response = await apiFetch('/api/settings'); if (!response.ok) return; const data = await response.json() as { context: AppContext; members: Member[]; invitations: Invitation[]; audit: AuditEvent[]; usage?: WorkspaceUsage; capabilities?: { aiConfigured: boolean }; workspaces?: Array<AppContext['workspace'] & { role: string }> }; setAppContext(data.context); setMembers(data.members); setInvitations(data.invitations); setAuditEvents(data.audit); setWorkspaceUsage(data.usage || { leads: 0, activeEvents: 0, knowledgeSources: 0, storageBytes: 0, activeMembers: 0 }); setCapabilities(data.capabilities || { aiConfigured: false }); setAvailableWorkspaces(data.workspaces || []); }
  ...
  function go(view: View) { setActiveView(view); setMobileNav(false); if (view === 'settings') void loadSettings(); if (view === 'knowledge') void loadKnowledge(); if (view === 'rfqs') { void loadRfqs(); void loadQuotations(); } if (view === 'roi') void loadEvents(); if (view === 'events') { void loadEvents(); void loadSettings(); } }
  ...
  function openReview(lead: SavedLead) {
    setActiveView('today'); setMobileNav(false); setReviewLead(lead); setAnalysis(null); setExtractionId(''); setAnalysisError(''); setConfirmed(false); setFollowups([]); void loadFollowups(lead.id);
  }
  ...
  function openApprovedFollowup(draft: FollowupDraft) {
    if (draft.status !== 'approved') return;
    const url = draft.channel === 'email' ? `mailto:${encodeURIComponent(draft.recipient)}?subject=${encodeURIComponent(draft.subject || '')}&body=${encodeURIComponent(draft.body)}` : `https://wa.me/${draft.recipient.replace(/\D/g, '')}?text=${encodeURIComponent(draft.body)}`;
  ...
  async function updateOpportunityStage(id: string, stage: string) {
    const response = await apiFetch('/api/workspace', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'update_opportunity', id, stage }) }); const data = await response.json() as { probability?: number; error?: string };
    if (!response.ok) { setNotice(data.error || 'Could not update opportunity'); return; } setOpportunities((current) => current.map((item) => item.id === id ? { ...item, stage, probability: data.probability ?? item.probability } : item)); setNotice('Opportunity stage updated'); void loadWorkspace();
  }
  ...
  async function confirmRfqExtraction(item: RfqItem) { if (!item.extractionId) return; setProcessingRfq(item.id); const response = await apiFetch('/api/rfqs/extract', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'confirm', rfqId: item.id, extractionId: item.extractionId }) }); const data = await response.json() as { error?: string }; setNotice(response.ok ? 'RFQ requirements confirmed' : data.error || 'Could not confirm RFQ extraction'); setProcessingRfq(''); await loadRfqs(); }
        ...
        <div className="content">
          ...
          <section className="welcome-row">
            ...
            <Dialog open={Boolean(reviewLead)} onOpenChange={(open) => { if (!open) setReviewLead(null); }}>
              <DialogContent className="review-dialog">
                ...
                {(confirmed || reviewLead?.reviewStatus === 'confirmed') ? <section className="followup-composer"><div><span><h3>Personalized follow-up</h3><p>AI drafts from confirmed facts only. Approval never sends the message.</p></span><div className="followup-buttons"><Button type="button" variant="outline" onClick={() => generateFollowup('email')} disabled={Boolean(drafting)}>{drafting === 'email' ? 'Drafting…' : 'Draft email'}</Button><Button type="button" variant="outline" onClick={() => generateFollowup('whatsapp')} disabled={Boolean(drafting)}>{drafting === 'whatsapp' ? 'Drafting…' : 'Draft WhatsApp'}</Button><Button type="button" onClick={() => { setOpportunityLead(reviewLead); setReviewLead(null); setOpportunityOpen(true); }}>Create opportunity</Button></div></div>{analysisError ? <p className="form-error" role="alert">{analysisError}</p> : null}{followups.map((draft) => <article key={draft.id}><span><b>{draft.channel}</b><small>To {draft.recipient} · {draft.status}</small></span>{draft.subject ? <strong>{draft.subject}</strong> : null}<p>{draft.body}</p><div><Button type="button" variant="outline" onClick={() => navigator.clipboard.writeText(draft.body)}>Copy</Button>{draft.status === 'approved' ? <Button type="button" variant="outline" onClick={() => openApprovedFollowup(draft)}>Open {draft.channel === 'email' ? 'email' : 'WhatsApp'}</Button> : null}<Button type="button" onClick={() => approveFollowup(draft.id)} disabled={draft.status !== 'draft'}>{draft.status === 'approved' ? <><Check /> Approved</> : 'Approve draft'}</Button></div></article>)}</section> : null}
              </DialogContent>
            </Dialog>
          </section>
          ...
          <section className="main-grid">
            <article className="panel action-panel">
              ...
              <div className="action-list">{tasks.filter((task) => task.status === 'open').length ? tasks.filter((task) => task.status === 'open').slice(0,5).map((task) => { const due = dueStatus(task.dueDate, appContext?.workspace.timezone || 'UTC'); return <button className="action-row" key={task.id} onClick={() => completeTask(task.id)}><span className="initial-avatar">{task.fullName.split(' ').map((word) => word[0]).join('').slice(0,2)}</span><span className="action-copy"><strong>{task.title}</strong><small>{task.fullName} · {task.company}</small></span><span className={`due ${due.tone}`}>{due.label}</span><Check className="row-arrow" size={17}/></button>; }) : <div className="empty-state">No open commitments. Capture a lead and add a next action.</div>}</div>
            </article>
          ...
          <section className="panel leads-panel">
            ...
            {activeView === 'opportunities' ? <article className="panel data-panel">{opportunities.length ? <><div className="data-header"><span>Opportunity</span><span>Stage</span><span>Value</span><span>Probability</span></div>{opportunities.map((item) => <div className="data-row" key={item.id}><span><strong>{item.title}</strong><small>{item.company}</small></span><select className="stage-select" aria-label={`Stage for ${item.title}`} value={item.stage} onChange={(event) => updateOpportunityStage(item.id, event.target.value)}><option value="qualified">Qualified</option><option value="requirement">Requirement</option><option value="sample">Sample</option><option value="rfq">RFQ</option><option value="quotation">Quotation</option><option value="meeting">Meeting</option><option value="negotiation">Negotiation</option><option value="won">Won</option><option value="lost">Lost</option></select><span>{money(item.value, item.currency)}</span><span>{item.probability}%</span></div>)}</> : <div className="empty-state large"><Target /><h2>No opportunities yet</h2><p>Convert a qualified conversation into your first pipeline record.</p><Button onClick={() => setOpportunityOpen(true)}>Create opportunity</Button></div>}</article> : null}
            ...
            {activeView === 'rfqs' && rfqs.some((item)=>item.documentCount) ? <section className="rfq-extraction-section"><div className="event-list-heading"><div><h2>Document extraction</h2><p>Machine-read requirements remain proposals until a team member confirms them.</p></div></div><div className="rfq-extraction-list">{rfqs.filter((item)=>item.documentCount).map((item)=>{const extracted=rfqExtraction(item.extractionJson);return <article className="panel rfq-extraction" key={item.id}><div><span><strong>{item.title}</strong><small>{item.requesterCompany}</small></span><b>{item.extractionStatus?.replaceAll('_',' ')||'not processed'}</b></div>{item.extractionStatus==='completed'&&extracted?<><p>{extracted.summary}</p>{extracted.items.slice(0,4).map((line,index)=><span key={`${line.product}-${index}`}>{line.product}{line.quantity?` · ${line.quantity}`:''}{line.specifications?` · ${line.specifications}`:''}</span>)}{extracted.warnings.length?<small>{extracted.warnings.join(' · ')}</small>:null}<Button type="button" onClick={()=>confirmRfqExtraction(item)} disabled={processingRfq===item.id}>{processingRfq===item.id?'Confirming…':'Confirm extracted requirements'}</Button></>:item.extractionStatus==='confirmed'?<p>Confirmed requirements are now part of the RFQ workflow.</p>:<Button type="button" variant="outline" onClick={()=>analyzeRfq(item.id)} disabled={processingRfq===item.id}>{processingRfq===item.id?'Extracting…':'Extract requirements'}</Button>}</article>})}</div></section>:null}
