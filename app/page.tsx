...
export default function Home() {
  ...
  async function loadSettings() { const response = await apiFetch('/api/settings'); if (!response.ok) return; const data = await response.json() as { context: AppContext; members: Member[]; invitations: Invitation[]; audit: AuditEvent[]; usage?: WorkspaceUsage; workspaces?: Array<AppContext['workspace'] & { role: string }> }; setAppContext(data.context); setMembers(data.members); setInvitations(data.invitations); setAuditEvents(data.audit); setWorkspaceUsage(data.usage || { leads: 0, activeEvents: 0, knowledgeSources: 0, storageBytes: 0, activeMembers: 0 }); setAvailableWorkspaces(data.workspaces || []); }
